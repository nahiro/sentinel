...
parser.add_option('-W','--trg_subset_width',default=WIDTH,type='int',help='Subset width in target pixel (%default)')
parser.add_option('-H','--trg_subset_height',default=HEIGHT,type='int',help='Subset height in target pixel (%default)')
...
template_half_width = opts.trg_subset_width//2
template_half_height = opts.trg_subset_height//2
...
for trg_indyc in [450]:
    ...
    for trg_indxc in [1250]:
        ...
        if ref_subset_data.min() < 1:
            continue
        p1 = np.array([0.0,0.0])
        result = leastsq(residuals,p1,args=(ref_subset_xp0,ref_subset_yp0,ref_subset_data,trg_subset_xp0,trg_subset_yp0,trg_subset_data),epsfcn=opts.feps,full_output=True)
        p2 = result[0]
